...
// ===============================
// RP PHONE BROWSER APP FULL
// drop-in module
// ===============================

(function () {

    if (window.RP_BROWSER_INSTALLED) return;
    window.RP_BROWSER_INSTALLED = true;
...
`;
    ...
    app.id = "rp-browser-app";

    app.innerHTML = `

<div class="rp-browser-top">

<input id="rp-browser-input" placeholder="Search..." />

<button id="rp-browser-search">
Search
</button>

<button id="rp-browser-close">
X
</button>

</div>

<div id="rp-browser-result">
...
`;
    ...
    async function callInternalAPI(prompt) {

        if (window.rpPhoneGenerate)
            return await window.rpPhoneGenerate(prompt);

        if (window.generateReply)
            return await window.generateReply(prompt);

        if (window.callAPI)
            return await window.callAPI(prompt);

        if (window.sendPrompt)
            return await window.sendPrompt(prompt);

        if (window.generateText)
            return await window.generateText(prompt);

        if (window.SillyTavern?.generate)
            return await window.SillyTavern.generate(prompt);

        throw "No internal API found";
    ...
    function addIcon() {
        ...
        if (!window.createAppIcon) return;

        window.createAppIcon({
